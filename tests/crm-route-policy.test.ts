...
test("public website and login dependencies stay public", () => {
  for (const path of ["/", "/register", "/forgot-password", "/reset-password", "/cars", "/cars/car-1", "/articles", "/contact", "/api/auth/login", "/api/auth/me", "/api/auth/self-register", "/api/auth/forgot-password", "/api/auth/reset-password", "/api/auth/reset-password/validate"]) {
...
test("admin and external routes are classified narrowly", () => {
  ...
  assert.equal(routeAccess("/api/cron/rdd-line-reminders"), "external");
  ...
  assert.equal(routeAccess("/api/internal/password-reset-email-sender-check"), "external");
});
